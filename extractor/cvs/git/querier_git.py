...
class GitQuerier():
    ...
    def get_diffs_manually(self, parent, commit, retrieve_patch):
        ...
        for line in lines:
            if flag:
                ...
                if not diff:
                    self.logger.warning("GitQuerier: diff empty for commit: " + commit.hexsha + " file_a: " + str(file_a) + " file_b: " + str(file_b))

            if re.match("^diff \-\-git", line):
                line_content = re.sub("^diff \-\-git ", "", line).strip()
    ...
    def get_status_no_diff(self, stats):
        additions = stats[0]
        deletions = stats[1]
